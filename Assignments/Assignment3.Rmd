...
```{r load-cities, eval=T, echo=TRUE}
...
head(cities_sf_3035)
st_crs(cities_sf_3035)
```


### Question 1: 
*What are the measurement units of the `cities_sf_3035` object?*
...
```{r inscriptions, eval=T, echo=TRUE}
...
inscriptions <- inscriptions %>% filter(lengths(coordinates) == 2) # Filter out empty coordinate data
i_sm <- inscriptions %>% 
slice(1:1000) %>% 
...
The location of the points looks very reasonable. The inscriptions are located in Europe, more specifically in mainland Italy.
...
```{r insc-sf, eval=FALSE, echo=TRUE}

# Since I already wrangled the dataset to filter out the missing values and split the coordinates into two deparate long, lat columns, I use the same code here, and add the 
# filtering by degrees smaller than 5 and larger than 20.

i <- inscriptions %>% filter(lengths(coordinates) == 2) %>% 
  mutate(coordinates = gsub("[^0-9.,]+", "", coordinates)) %>%
  separate(col = coordinates, into = c("longitude","latitude"), sep = ',') %>%
  mutate(latitude = as.numeric(latitude),
         longitude = as.numeric(longitude)) %>%
...
```{r intersection, eval=FALSE, echo=TRUE}
...
There are 1608 rural inscriptions vs 10462 urban inscriptions.
